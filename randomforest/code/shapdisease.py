...
for i in df[var].unique():
    ...
    shaps[i] = pd.Series(explainer(X).values.mean(axis=0)[:,0], index=X.columns)
...
plotdf = shaps[(np.abs(stats.zscore(shaps)) > 8).any(axis=1)]
plotdf = shaps[(np.abs(stats.zscore(shaps)) > 4.5).any(axis=1)]
