...
#metamsptaxo = regionmetamsptaxo.groupby('westernised').mean().T
...
dft = regionmetamsptaxo.drop('Geography', axis=1).groupby('westernised').mean().T
nwvals = dft.loc[dft.sort_values('NW').tail(20).index]
#nwvals = dft.loc[sortmat.sort_values('NW').tail(20).index]
#wvals = dft.loc[sortmat.sort_values('W').tail(20).index]
wvals = dft.loc[dft.sort_values('W').tail(20).index]
...
sns.heatmap(zscores.loc[(zscores.T > 3).any()],xticklabels=True, yticklabels=True, cmap='coolwarm')
...
def sig(df, mult=False):
    ...
    i,j  = combs[0], df.columns[0]
    out = [mannwhitneyu(df.loc[i,j], df.loc[df.index.drop(i), j])[1] for i in df.index.unique() for j in df.columns]
    outdf = pd.DataFrame(
        [mannwhitneyu(df.loc[i,j], df.loc[df.index.drop(i), j])[1] for i in df.index.unique() for j in df.columns],
        columns = df.columns,
        index = combs
        ).T
    if mult:
    ...
    return outdf
# sig between countries
df = regionmetamsptaxo.drop('westernised',axis=1).set_index('Geography')
result = sig(df,mult=True)
