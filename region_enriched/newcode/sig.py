...
with open('tmp.csv', 'w') as of: of.write("".join(str(out)))
cols =  [[i, j] for i in df.index.unique() for j in df.columns]
asdf = pd.DataFrame(out, index=pd.DataFrame(cols).set_index([0,1]).index)
asdfg = asdf.unstack()
asdfg.to_csv('allpvals.csv')
